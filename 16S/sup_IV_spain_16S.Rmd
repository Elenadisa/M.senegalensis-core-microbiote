---
title: "Spain (Understory vs Gap) Analysis 16S"
...
In this analysis we want to analyse the differences in microbiota composition between IN and OUT soil conditions in Spain.
...
ps.spain <- subset_samples(physeq.clean, Site == "Spain")
...
#remove Phylum not in all samples
...
```{r AbundancePerSampleHeatmapGenusAllSamples, fig.cap="Heatmap comparing abundance of genera across experimental groups. Color gradients representing the abundance. Each row corresponds to a genus, while each column represents a sample grouped by experimental condition."}
...
"For a particular gene, a log2 fold change of −1 for condition treated vs untreated means that the treatment induces a change in observed expression level of 2^−1 = 0.5 compared to the untreated condition."  This mean, that positive log2 fold change means that the genus is more abundant under IN condition, whereas, negative log2 fold change means that the genus is more abundant under Gap condition.

If we analyse the genera that are within all the samples in Spain and Senegal under the tree (see supplementary file II),  Candidatus nitrocosmicus and TRAS3-20 genera are significantly more abundant in Understory condition. Blastococcus and WD2101_soil_group, are more abundant in Gap condition.  
...
if(is.null(lefse.all@marker_table)){
  print("No marker were identified")
}else{
...
```{r Genus-1Samples}
#remove Genus not in all samples except one
...
```{r AbundancePerSampleHeatmapGenus-1Samples, fig.cap="Heatmap comparing abundance of genera across experimental groups. Color gradients representing the abundance. Each row corresponds to a genus, while each column represents a sample grouped by experimental condition."}
...
```{r AbundanceHeatmapGenus-1Samples, fig.cap="Heatmap comparing abundance of genera across experimental groups. Color gradients representing the abundance. Each row corresponds to a Genus, while each column represents an experimental group, facilitating the comparison of genus distribution and abundance patterns across groups."}
...
Candidatus nitrocosmicus is more abundant in Understory condition. Blastococcus and WD2101_soil_group, are more abundant in Gap condition.   
