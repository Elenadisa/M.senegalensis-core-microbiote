...
ggplotly(ggplot(readcount, aes(TotalReads)) + geom_histogram() + ggtitle("Sequencing Depth") + facet_wrap(Site~INorOUT))
```  
...
ggplot(phylum_sumarize, aes(x="", y=Abundance_mean, fill=Phylum)) +
  ...
  theme_void()+ 
  coord_polar("y", start=0)  + facet_wrap(Site~INorOUT, scales = "free")
...
```{r AbunByTopGenus, fig.height=8, fig.cap="Abundance of the top 20 mosta abundant genera"}
plot_abundance_by_taxa(top_genus_sumarize, "Genus", "Abundance", "INorOUT") + facet_wrap(~Site, ncol=1, scales = "free")
...
pairwise_bray$`Senegal_Understory_vs_Senegal_Gap`
...
#pairwase adonis
pairwise_bray$`Spain_Understory_vs_Spain_Gap`
```
...
#pairwase adonis
pairwise_bray$`Spain_Understory_vs_Senegal_Understory`
```
...
plot_distance(physeq.10, pcoa_bray, "INorOUT", shape="Site", elipse = TRUE) + annotate("text" ,x=0.2,y=0.43, label = bray_permanova_text) 
```
...
```{r AllVenDiagram, fig.width=10, fig.cap="Venn Diagram of Genus within all the samples in the different conditions."}
#Create the list
...
ggVennDiagram(lst1, label_alpha = 0) +  scale_fill_gradient(low = "#F4FAFE", high = "#4981BF") +
  theme(legend.position = "none" ) 
```
  
We can see that there are different genera that are shared by the different conditions.
  
```{r UnderstoryVenDiagram, fig.width=10, fig.cap="Venn Diagram of Genus within all the samples in the Understory conditions."}
#Create the list
lst2 <- list(Sen= senegal.in.genus_l,
            Spa= spain.in.genus_l)

ggVennDiagram(lst2, label_alpha = 0) +  scale_fill_gradient(low = "#F4FAFE", high = "#4981BF") +
  theme(legend.position = "none" ) 
```
