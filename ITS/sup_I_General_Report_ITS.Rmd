...
top_genus_sumarize_ordered <- top_genus_sumarize %>%
  group_by(Site, INorOUT) %>%
  mutate(Order = ifelse(INorOUT == "Understory", rank(Absolute_Abundance, ties.method = "first"), 
...
top_genus_sumarize_ordered <- top_genus_sumarize_ordered %>%
  mutate(Genus_microhabitat = paste(Genus, INorOUT, Site, sep = "_"))
...
top_genus_fungi <- plot_abundance_by_taxa(top_genus_sumarize_ordered, "Genus_microhabitat", "Absolute_Abundance", "INorOUT") + theme(strip.text = element_text(size = 20, face = "bold")) + scale_x_discrete(labels = function(x) gsub("_Understory_Senegal|_Understory_Spain|_Gap_Senegal|_Gap_Spain", "", x)) +  facet_wrap(~Site, scales = "free") +
...
#Chao Plot
alpha_fungi_senegal <- alpha_diversity_plot(physeq.senegal, "INorOUT", paper_metrics, my_comparisons, stat_metrics, asterisk = FALSE, " ") + theme(strip.text = element_blank(), axis.title.y=element_blank())
...
#Chao Plot
alpha_fungi_spain <- alpha_diversity_plot(physeq.spain, "INorOUT", paper_metrics, my_comparisons, stat_metrics, asterisk = FALSE, " ") + theme(strip.text = element_blank(), axis.title.y=element_blank())
...
#Chao Plot
alpha_fungi_under <- alpha_diversity_plot(physeq.in, "Site", paper_metrics, my_comparisons, stat_metrics, asterisk = FALSE, " ") + theme(strip.text = element_blank(), axis.title.y=element_blank())
...
#Chao plot
alpha_fungi_gap <- alpha_diversity_plot(physeq.out, "Site", paper_metrics, my_comparisons, stat_metrics, asterisk = FALSE, " ") + theme(strip.text = element_blank())
...
beta_fungi <- plot_distance(physeq.10, pcoa_bray, "INorOUT", shape="Site", elipse = TRUE) + annotate("text" ,x=0.2,y=0.43, label = bray_permanova_text, size = 5) 
beta_fungi
```
