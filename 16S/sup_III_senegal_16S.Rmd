---
title: "Senegal (Understory vs Gap) Analysis 16S"
...
In this analysis we want to analyse the differences in microbiota composition between Understory and Gap soil conditions in Senegal.
...
Then, we obtain only Senegal samples:  
  
```{r ObtainSenegalSamples}
ps.senegal <- subset_samples(physeq.clean, Site == "Senegal")
#remove ASVs with a total abundance 1 in all samples
physeq.senegal.clean <- prune_taxa(taxa_sums(ps.senegal) > 1, ps.senegal)
...
physeq.css <- normalize(physeq.senegal.clean, "CSS")
```
...
genus_sumarize.all <- genus_df.all %>%  group_by(INorOUT, Genus) %>% 
...
phylum_sumarize.all <- phylum_df.all %>%  group_by(INorOUT, Phylum) %>% 
  summarise(Abundance=mean(Abundance, na.rm=TRUE), sd=sd(Abundance, na.rm=TRUE))
...
```{r AbundancePerSampleHeatmapPhylymAllSamples, fig.cap="Heatmap comparing abundance of phyla across experimental groups. Color gradients representing the abundance. Each row corresponds to a phylum, while each column represents a sample grouped by experimental condition."}

ggplotly(plot_abundance_heatmap(phylum_df.all, "Abundance", "Phylum", "Sample") + facet_wrap(~INorOUT, scales = "free_x"))
```

**At Genus level**

```{r AbundancePerSampleHeatmapGenusAllSamples, fig.height = 8, fig.cap="Heatmap comparing abundance of genera across experimental groups. Color gradients representing the abundance. Each row corresponds to a genus, while each column represents a sample grouped by experimental condition."}

ggplotly(plot_abundance_heatmap(genus_df.all, "Abundance", "Genus", "Sample") + facet_wrap(~INorOUT, scales = "free_x"))
```
...
```{r AbundanceHeatmapPhylymINOUT, fig.cap="Heatmap comparing abundance of phyla across experimental groups. Color gradients representing the abundance. Each row corresponds to a phylum, while each column represents an experimental group, facilitating the comparison of phylum distribution and abundance patterns across groups."}

ggplotly(plot_abundance_heatmap(phylum_sumarize.all, "Abundance", "Phylum", "INorOUT"))
```

**At Genus level**

```{r AbundanceHeatmapGenusINOUT, fig.height = 8, fig.cap="Heatmap comparing abundance of genera across experimental groups. Color gradients representing the abundance. Each row corresponds to a Genus, while each column represents an experimental group, facilitating the comparison of genus distribution and abundance patterns across groups."}

ggplotly(plot_abundance_heatmap(genus_sumarize.all, "Abundance", "Genus", "INorOUT"))
```

## Differential Abundance  
  
To see if there are significant differences in the abundance of the genera in both conditions, we have performed a Differential abundance analysis. Due there are a lot of algorithms to do this analysis and we can obtain different results depending on the chosen algorithm, we have performed an analysis using two different approaches. DESeq2, an RNA-seq method, and LefSE, a method developed for metagenomic analyses.
...
```{r DESeqAll}
#items to compare
contrast <- c("INorOUT","Understory", "Gap")
#P.adjust threshold
alpha <- 0.05 

#create DESeq2 object
ds.all <- phyloseq_to_deseq2(ps.genus.all, ~ INorOUT)
...
If we analyse the genera that are within all the samples in Senegal (see supplementary file II), Steroidobacter, Pir4_lineage, Nitrospira, and MND1 are significantly more abundant in Understory condition, whereas, Microvirga and Ramlibacter are significantly more abundant in Gap condition.  
...
LEfSe only found Microvirga Genus as significantly more abundant under Gap condition.  
...
genus_sumarize.90 <- genus_df.90 %>%  group_by(INorOUT, Genus) %>% 
  summarise(Abundance=mean(Abundance, na.rm=TRUE), sd=sd(Abundance, na.rm=TRUE))
...
With this plot we want to see the microbial composition in all samples except 1 and ensure that the ASVs are within samples en each  experiment group. Moreover, we can see if there is any difference in the abundance of the ASVs in the different experimental conditions.

**At Genus level**

```{r AbundancePerSampleHeatmapGenus-1Samples, fig.height=8, fig.cap="Heatmap comparing abundance of genera across experimental groups. Color gradients representing the abundance. Each row corresponds to a genus, while each column represents a sample grouped by experimental condition."}

ggplotly(plot_abundance_heatmap(genus_df.90, "Abundance", "Genus", "Sample") + facet_wrap(~INorOUT, scales = "free_x"))
```
...
```{r AbundanceHeatmapGenus-1Samples, fig.height=8, fig.cap="Heatmap comparing relative abundance of genera across experimental groups.Color gradients representing the abundance. Each row corresponds to a Genus, while each column represents an experimental group, facilitating the comparison of genus distribution and abundance patterns across groups."}

ggplotly(plot_abundance_heatmap(genus_sumarize.90, "Abundance", "Genus", "INorOUT"))
```
...
#create DESeq2 object
ds.90 <- phyloseq_to_deseq2(ps.genus.90, ~ INorOUT)
...
If we analyse the genera that are within all the samples in Spain and Senegal under the tree (see supplementary file II), Nitrospira, Pir4_lineage, MND1, bacteriap25, and Acidibacter are significantly more abundant in Understory condition, whereas, Microvirga, Ramlibacter, Geodermatophilus and Flavisolibacter are significantly more abundant under Gap condition. 
...
We have obtained the same results than in the first approach.  
